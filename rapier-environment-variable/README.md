...
First, add Rapier and Dagger to your build. This involves adding dependencies, annotation processors, and a generated source folder. Ensure the environment variable annotation processor runs before the Dagger annotation processor.
...
This will generate a new module, `RapierExampleComponentEnvironmentVariableModule`, for `ExampleComponent` in the same package. Note that the example component references the generated module by name. Generating the module isn't useful if it isn't added to the component!
