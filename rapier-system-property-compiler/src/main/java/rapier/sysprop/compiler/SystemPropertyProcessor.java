...
import rapier.sysprop.compiler.model.ParameterMetadata;
import rapier.sysprop.compiler.model.RepresentationKey;
import rapier.sysprop.compiler.util.SystemProperties;
...
public class SystemPropertyProcessor extends RapierProcessorBase {
  ...
  private List<DaggerInjectionSite> computeRelevantInjectionSites(TypeElement component) {
    return new DaggerComponentAnalyzer(getProcessingEnv())
        ...
        .filter(d -> {
          ...
          if (!isValidSystemPropertyNameTemplate(parameter.getName())) {
            getMessager().printMessage(Diagnostic.Kind.ERROR,
                "Invalid system property name template", d.getElement());
            return false;
          }
          return true;
        }).collect(toList());
  ...
  private boolean isValidSystemPropertyNameTemplate(String template) {
    final AtomicBoolean result = new AtomicBoolean(true);
    try {
      new TemplateParser().parse(template, new TemplateParser.ParseHandler() {
        @Override
        public void onText(int index, String text) {
          if (!text.chars().allMatch(SystemPropertyProcessor.this::isValidSystemPropertyNameChar)) {
            result.set(false);
          }
        }
  ...
  private boolean isValidSystemPropertyNameChar(int ch) {
    ...
    for (Map.Entry<ParameterKey, List<DaggerInjectionSite>> entry : injectionSitesByParameter
      ...
      if (injectionSitesByRequired.size() > 1) {
        getMessager().printMessage(Diagnostic.Kind.WARNING,
            "Conflicting requiredness for system property " + key.getName()
   ...
   * realities, e.g., treating a required system property as nullable.
   * 
      ...
      for (DaggerInjectionSite injectionSite : representationInjectionSites) {
        ...
        if (parameterIsRequired && injectionSiteIsNullable) {
          getMessager().printMessage(
              Diagnostic.Kind.WARNING, "Effectively required system property "
  ...
  private String generateSourceCode(String componentPackageName, String moduleClassName,
    ...
    try (final PrintWriter writer = new PrintWriter(result)) {
      ...
      writer.println("    value = \"" + SystemPropertyProcessor.class.getCanonicalName() + "@"
          + version + "\",");
  ...
  private TypeMirror getQualifierType() {
    if (qualifierType == null) {
      qualifierType = getElements().getTypeElement(SystemProperty.class.getName()).asType();
    }
