...
public class SystemPropertyProcessor extends AbstractProcessor {
  ...
  private void processComponent(TypeElement component) {
    ...
    final SortedMap<String, Set<TypeMirror>> environmentVariables =
        analysis.getDependencies().stream().filter(d -> d.getQualifier().isPresent())
            ...
            .collect(groupingBy(d -> extractSystemProperty(d.getQualifier().get()), TreeMap::new,
    ...
    try {
      ...
      try (final PrintWriter writer = new PrintWriter(o.openWriter())) {
        ...
        for (Map.Entry<String, Set<TypeMirror>> e : environmentVariables.entrySet()) {
          ...
          writer.println("    @SystemProperty(\"" + environmentVariableName + "\")");
          writer.println("    public String " + baseMethodName + "() {");
          writer.println("        return props.get(\"" + environmentVariableName + "\");");
          writer.println("    }");
          writer.println();
          ...
          for (TypeMirror requiredType : requiredTypes) {
            ...
            writer.println(
                "    public " + requiredType + " " + baseMethodName + "As" + requiredTypeSimpleName
                    + "(@SystemProperty(\"" + environmentVariableName + "\") String value) {");
  ...
  private String extractSystemProperty(AnnotationMirror annotation) {
